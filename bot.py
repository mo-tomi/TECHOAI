...
async def handle_vc_selfcheck_reminder(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    try:
        await after.channel.send(
            ...
            "回答内容に応じてLv2になると、画像の添付や画面共有、"
            "音楽bot（Jockie Music）の操作ができるようになりますよ✨\n"
            "※VC内でメッセージが送信できるのはLv2からです"
        )
    except discord.HTTPException:
...
async def setup_selfcheck_command(interaction: discord.Interaction):
    ...
    if cooldown_days > 0:
        retry_note = (
            f"合格に至らなかった場合も{max_attempts}回まで挑戦できます。"
            f"{max_attempts}回とも届かなかったときは、{cooldown_days}日後にまた挑戦できます🌸"
        )
    else:
    ...
    embed.add_field(
        ...
        value=(
            ...
            "\n"
            "**Lv1（全員）**\n"
            "サーバーに入ると全員に付くロール。一部制限はありますが（VC内チャットでのメッセージ送信不可など）、"
            "VC参加・発言・読み上げbotの利用は問題なくできます\n"
            "\n"
            "**Lv2（一般）**\n"
            "セルフチェックの回答内容に応じて自動付与。画像/ファイル添付、VC内でのテキスト送信、画面共有、"
            "音楽bot（Jockie Music）の操作などができるようになります\n"
            "\n"
            "**Lv3**\n"
            "管理人・副管理人が信頼できると判断したメンバーに手動で付与するロール（セルフチェック対象外）。"
            "寝落ちしているメンバーの移動ができるようになります\n"
            "\n"
